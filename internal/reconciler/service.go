...
func BuildService(gs *gamesv1alpha1.GameServer) *corev1.Service {
...
func BuildHeadlessService(gs *gamesv1alpha1.GameServer) *corev1.Service {
	ports := make([]corev1.ServicePort, 0, len(gs.Spec.Network.Ports))
	for _, p := range gs.Spec.Network.Ports {
		sp := corev1.ServicePort{
			Name:       p.Name,
...
func GameServerLabels(gs *gamesv1alpha1.GameServer) map[string]string {
	return map[string]string{
		...
		"games.gobehost.com/game-type": gs.Spec.Game.Type,
		"games.gobehost.com/game-id":   gs.Name,
	}
}
